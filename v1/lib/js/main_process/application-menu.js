...
function export_files() {
  ...
  var targets = ['load_target.json', 'theme.json', 'lib/css/custom.css', 'lib/js/custom.js', 'lib/js/override/reveal.js/plugin/markdown/markdown.js'];
...
function copy_dependent_nodemodules(export_to_dir) {
  ...
  if(!fse.existsSync(export_to_dir + '/node_modules/uikit/')) {
    let target_dirs = ['css', 'js'];
    for( var i = 0; i < target_dirs.length; i++ ) {
      var uikit_from = settings.get('app.root_dir') + '/node_modules/uikit/dist/' + target_dirs[i];
      var uikit_to   = export_to_dir + '/node_modules/uikit/dist/' + target_dirs[i];
      fse.copySync(uikit_from , uikit_to, { dereference: true });
    }
  }
}
