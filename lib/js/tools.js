      ...
      $('.js-file-select-dialog').on("click", function() {
        ...
        const callback = function(selected_dir_paths) {
          if(selected_dir_paths != undefined) {
            ...
            set_target_element.val(selected_dir_paths[0]);
          } else {
...
function create_editor_theme_select_box_html() {
  ...
  var fileList = fs.readdirSync(settings.get('app.root_dir') + '/node_modules/ace-min-noconflict/');
  for (var i = 0; i < fileList.length; i++) {
    if( fileList[i].match(/theme-.*\.js/)) {
      var theme_name = fileList[i].match(/theme-(.*)\.js/)[1];
...
function create_editor_keibind_select_box_html() {
  ...
  fileList.unshift('keybinding-ace.js');

  for (var i = 0; i < fileList.length; i++) {
    if( fileList[i].match(/keybinding-.*\.js/)) {
      var keybinding_name = fileList[i].match(/keybinding-(.*)\.js/)[1];
  ...
  vex.open({
    unsafeContent: [
      '<div>',
        ...
        '<progress id="js-download-progress" class="uk-width-8-10 uk-progress" value="30" max="100"></progress>',
        '<div class="uk-width-1-10"></div>',
      '</div>',
    ].join(''),
    showCloseButton: false,
