  ...
  vex.open({
    ...
    afterOpen: function() {
      ...
      $("#js-change-theme").on("change", function() {
        fs.writeFileSync(settings.get('app.root_dir') + '/theme.json', '{ "theme_css_path": "node_modules/reveal.js/css/theme/' + this.value + '", "theme_css_filename": "' + this.value + '"}');
...
function create_theme_select_box_html() {
  var theme_json_str = fs.readFileSync(settings.get('app.root_dir') + '/theme.json', 'UTF-8');
...
function create_file_links() {
  var file_links = [];
  var dirList = fs.readdirSync(settings.get('app.work_dir'));
...
function open_load_file_dialog() {
  var file_links = create_file_links();
  vex.dialog.open({
    input: [
      '<div class="uk-grid">',
        file_links.join(''),
