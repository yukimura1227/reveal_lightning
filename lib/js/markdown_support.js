...
function adjust_markdown_preview_scroll() {
  ...
  var currentLine = editorDocument.getLines(cursorPosition.row, cursorPosition.row);
  ...
  for( var i = 0; i < upperLines.length; i++ ) {
    if( upperLines[i] === '----------' || upperLines[i] === '---' ) {
      hr_count++;
    }
  }
  var currentTop = $('#js-markdown-result').scrollTop();
  var pos = $($('#js-markdown-result').find('hr')[hr_count]).position()
  $('#js-markdown-result').scrollTop(currentTop + pos.top);
}
