...
window.addEventListener('load',function() {
  ...
  $('#js-image-paste-btn').on('click', function() {
    ...
    setTimeout(
      function () {
        RevealLightning.utility.put2editor("![](" + settings.get('target_md.file_relative_dir') + '/' + image_file_name + ")\n");
  ...
  $('#js-chapter-separate-btn').on('click', function() {
    RevealLightning.utility.put2editor("----------\n");
  ...
  $('#js-fragment-btn').on('click', function() {
    RevealLightning.utility.put2editor('<!-- .element: class="fragment" -->');
  });

  $('#js-fragment-grow-btn').on('click', function() {
    RevealLightning.utility.put2editor('<!-- .element: class="fragment grow" -->');
  });

  $('#js-fragment-shrink-btn').on('click', function() {
    RevealLightning.utility.put2editor('<!-- .element: class="fragment shrink" -->');
  });

  $('#js-fragment-fade-up-btn').on('click', function() {
    RevealLightning.utility.put2editor('<!-- .element: class="fragment fade-up" -->');
  });

  $('#js-fragment-fade-down-btn').on('click', function() {
    RevealLightning.utility.put2editor('<!-- .element: class="fragment fade-down" -->');
  });

  $('#js-fragment-fade-left-btn').on('click', function() {
    RevealLightning.utility.put2editor('<!-- .element: class="fragment fade-left" -->');
  });

  $('#js-fragment-fade-right-btn').on('click', function() {
    RevealLightning.utility.put2editor('<!-- .element: class="fragment fade-right" -->');
  });

  $('#js-fragment-current-visible-btn').on('click', function() {
    RevealLightning.utility.put2editor('<!-- .element: class="fragment current-visible" -->');
  });

  $('#js-fragment-highlight-current-blue-btn').on('click', function() {
    RevealLightning.utility.put2editor('<!-- .element: class="fragment highlight-current-blue" -->');
  });

  $('#js-fragment-highlight-red-btn').on('click', function() {
    RevealLightning.utility.put2editor('<!-- .element: class="fragment highlight-red" -->');
  });

  $('#js-style-full-width-btn').on('click', function() {
    RevealLightning.utility.put2editor('<!-- .element: style="width: 100%" -->');
  });
});
  ...
  vex.open({
      ...
      '<form class="vex-dialog-form">',
        '<div class ="vex-dialog-input">',
          '<div class="uk-grid">',
            ...
            '<div class="uk-width-1-2">',
              create_editor_keibind_select_box_html().join(''),
            '</div>',
            '<div class="uk-width-1-4"></div>',

            '<label class="uk-width-1-4">workspace</label>',
            '<div class="uk-width-1-2">',
              '<input type="text" id="js-config-work-dir" value="' + settings.get('env.work_dir') + '" readonly/>',
            '</div>',
            '<div class="uk-width-1-4">',
            ...
            '<div class="uk-width-1-2">',
              '<input type="text" id="js-config-export-to" value="' + settings.get('export.to') + '" readonly/>',
            '</div>',
            '<div class="uk-width-1-4">',
    ...
    afterOpen: function() {
      ...
      $("#js-change-editor-theme").on("change", function() {
        var theme = $(this).val();
        RevealLightning.markdown_support.set_editor_theme(theme);
      });
      $("#js-change-editor-keybinding").on("change", function() {
        var keybinding_name = $(this).val();
        RevealLightning.markdown_support.set_editor_keybinding(keybinding_name);
      });
      ...
      $('.js-file-select-dialog').on("click", function() {
        ...
        const callback = function(selected_dir_paths) {
          if(selected_dir_paths != undefined) {
            console.log(selected_dir_paths);
            set_target_element.val(selected_dir_paths[0]).change();
          } else {
    ...
    callback: function (data) {
      if (!data) {
        ...
        if(!fs.existsSync(write_file_dir)) {
          ...
          RevealLightning.markdown_support.load_markdown_file();
          $('#js-reload-reveal-view-btn').trigger('click');
...
function create_editor_keibind_select_box_html() {
...
function create_file_links() {
  ...
  for (var i = 0; i < dirList.length; i++) {
    file_links.push('<div class="uk-width-1-1"><a href="javascript:void(0);" onclick="RevealLightning.tools.set_file_key(\'' + dirList[i] + '\');">' + dirList[i] + '</a></div>');
  }
  return file_links;
}

export function set_file_key(keyword) {
  $('#js-open-file-keyword').val(keyword);
}
...
function create_file_link_table_tag() {
  ...
  for (var i = 0; i < dirList.length; i++) {
    ...
    table_tag.push('        <td onclick="RevealLightning.tools.set_file_key(\'' + dirList[i] + '\');">' + dirList[i] + '</td>');
    table_tag.push('      </tr>');
  }
  table_tag.push('    </tbody>');
