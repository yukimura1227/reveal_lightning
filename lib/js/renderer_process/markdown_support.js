...
export function setup_markdown() {
  ...
  editor.getSession().getDocument().on('change', function() {
    ...
    RevealLightning.utility.write_file(settings.get('target_md.file_path'));
...
export function set_editor_keybinding(keybinding_name) {
...
export function load_markdown_file() {
  var text = RevealLightning.utility.read_file(settings.get('target_md.file_path'));
