...
function export_files() {
  var export_to_dir = server_root() + '/tmp'
  copy_dependent_nodemodules(export_to_dir);
  fse.copySync(server_root() + '/reveal_view.html', export_to_dir + '/index.html');
  // TODO: コピー先のindex.htmlの参照パスを書き換える
  var targets = ['load_target.json', 'theme.json', 'lib/css/custom.css', 'lib/js/custom.js'];
  for( var i = 0; i < targets.length; i++ ) {
    fse.copySync(server_root() + '/' + targets[i], export_to_dir + '/' + targets[i]);
  }
  var markdown_from = server_root() + '/' + settings.get('target_md.file_relative_path');
  var markdown_to   = export_to_dir + '/' + settings.get('target_md.file_relative_path');
  console.log(markdown_from);
  console.log(markdown_to);
...
function copy_dependent_nodemodules(export_to_dir) {
  ...
  if(!fse.existsSync(export_to_dir + '/node_modules/jquery')) {
    ...
    fse.copySync(jquery_from , jquery_to + '/jquery.min.js');
  }
  if(!fse.existsSync(export_to_dir + '/node_modules/mathjax')) {
    ...
    for( var i = 0; i < target_dirs.length; i++ ) {
      ...
      fse.copySync(mathjax_from , mathjax_to);
    }
  }
  if(!fse.existsSync(export_to_dir + '/node_modules/reveal.js')) {
    ...
    for( var i = 0; i < target_dirs.length; i++ ) {
      ...
      fse.copySync(revealjs_from , revealjs_to);
    }
  }
}
